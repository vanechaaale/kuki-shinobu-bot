...
class JsonManager {
    ...
    constructor(data, readonly = false, fastMode = false, route = []) {
        this._data = data;
    ...
    getAsNullableJsonObject(key) {
        const value = this.getValue(key);
        if (!(0, json_utils_1.isJsonObject)(value) && value !== null)
            throw new InvalidTypeError(value);
        return value;
    }
    getAsNullableJsonArray(key) {
        const value = this.getValue(key);
        if (!(0, json_utils_1.isJsonArray)(value) && value !== null)
            throw new InvalidTypeError(value);
        return value;
    }
    get(...keys) {
    ...
    detach() {
        return new JsonManager((this.fastMode ? this._currentData : this.getValue()), this.readonly, this.fastMode);
    }
    asObject() {
        const dataHere = this.getAs();
